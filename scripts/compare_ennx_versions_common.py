...
def _import_optimizer_configs() -> dict[str, Any]:
    try:
        ...
        from ennx.turbo.optimizer_config import (
            AcqType,
            CandidateGenConfig,
            ENNFitConfig,
            ENNSurrogateConfig,
...
def _make_candidate_gen_config(num_candidates: int | None = None) -> Any:
    ...
    if callable(getattr(probe, "num_candidates", None)):
        try:
            from ennx.turbo.config.num_candidates_fn import const_num_candidates
        except ImportError:
...
def separable_unimodal_objective(x: np.ndarray) -> np.ndarray:
    try:
        from ennx.benchmarks.separable_unimodal import (
