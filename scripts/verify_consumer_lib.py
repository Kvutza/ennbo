...
def test_end_to_end_quantized_search():
    ...
    for backend in ["cpu", "metal"]:
        ...
        trial_index, trial_seed, trial_score = search.ask(np.array([19, 23, 29, 31], dtype=np.uint64), 0.65, 1)
        print(f"  [{backend}] Winning trial index: {trial_index}, seed: {trial_seed}, score: {trial_score:.4f}")
