# Metal and OpenCL ENNBO

Status: design note

This document describes the next ENNBO backend. The target is a Rust ENNBO
...
The target is a device-resident ENNBO path. Rust remains responsible for
...
the ENNBO crate. The fork must therefore not blindly merge upstream and lose
...
- rewrite the Granite or Olmoe model evaluator inside ENNBO;
...
version. The ENNBO optimizer can become GPU-resident before the model
...
- Reintroduce `IndexDriver::Metal` through the current ENNBO backend.
