...
from scripts.compare_ennx_versions import (
    BASELINE_LABEL,
    CURRENT_LABEL,
    _build_parser,
    _compare_summary,
...
def test_run_worker_subprocess_dispatch(monkeypatch):
    from scripts import compare_ennx_versions as module
...
def test_main_quick_smoke(monkeypatch, tmp_path):
    ...
    rc = main(["--quick"])
    assert rc == 0
    report = json.loads((tmp_path / "compare_ennx_versions_report.json").read_text())
