...
use ennx::traits::PosteriorComputation;
...
impl PyEpistemicNearestNeighbors {
    ...
    fn new(
        ...
        let storage = match enn_storage {
            Some("disk" | "Disk") => ennx::EnnStorage::Disk,
            Some("memory" | "in_memory" | "InMemory") => ennx::EnnStorage::InMemory,
            None if work_dir.is_some() => ennx::EnnStorage::Disk,
            None => ennx::EnnStorage::InMemory,
            Some(other) => {
                return Err(PyValueError::new_err(format!(
    ...
    fn batch_posterior<'py>(
        ...
        for i in 0..n_params {
            let params =
                ennx::ENNParams::new(k_values[i], epistemic_scales[i], aleatoric_scales[i])
                    .map_err(|e| PyValueError::new_err(e.to_string()))?;
            paramss.push(params);
        }
...
mod kiss_coverage_tests {
    ...
    fn py_model_units_are_linked() {
        let _ = py_posterior_flags
            as fn(&PyEpistemicNearestNeighbors, bool, bool) -> ennx::PosteriorFlags;
        let _ = (
