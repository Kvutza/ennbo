...
pub fn sobol_sequence_py<'py>(
    ...
    let mut engine = ennx::candidates::SobolEngine::new(dimension)
...
pub fn set_config_path_py(path: Option<&str>) -> PyResult<()> {
    ennx::set_config_path(path.map(std::path::PathBuf::from));
    Ok(())
}

/// Ensure `~/.ennx/config.toml` (or override) exists and return its path.
#[pyfunction(name = "ensure_config_file")]
pub fn ensure_config_file_py() -> PyResult<String> {
    ennx::install_bpann_tuning_from_config();
    let cfg = ennx::Config::new();
    let _ = cfg.load();
