...
fn check_append_row_limit() {
    ennx::backend::disk_observation::check_append_row_limit(10).unwrap();
}

#[test]
fn read_index_stale() {
    let stale = Mutex::new(false);
    assert!(!ennx::backend::disk_observation::read_index_stale(&stale));
    ennx::backend::disk_observation::set_index_stale(&stale);
    assert!(ennx::backend::disk_observation::read_index_stale(&stale));
}
...
fn open_or_append_yvar() {
    ...
    let yv = array![[0.1]];
    ennx::backend::disk_observation::open_or_append_yvar(dir.path(), 1, Some(&yv)).unwrap();
}
...
fn validate_index_backend() {
    ...
    ennx::backend::disk_observation::write_metadata(dir.path(), 0, 4, 1, false, 0, "bpann_disk")
        .unwrap();
    ennx::backend::disk_observation::validate_index_backend(dir.path(), "bpann_disk").unwrap();
}
...
fn mark_index_dirty() {
    ...
    ennx::backend::disk_observation::mark_index_dirty(&dirty);
...
fn load_indexed_rows() {
    ...
    ennx::backend::disk_observation::write_metadata(dir.path(), 0, 4, 1, false, 2, "bpann_disk")
        .unwrap();
    assert_eq!(
        ennx::backend::disk_observation::load_indexed_rows(dir.path()),
        Some(2)
    );
}
...
fn load_index_backend() {
    ...
    ennx::backend::disk_observation::write_metadata(dir.path(), 0, 4, 1, false, 0, "bpann_disk")
        .unwrap();
    assert_eq!(
        ennx::backend::disk_observation::load_index_backend(dir.path()).as_deref(),
        Some("bpann_disk")
    );
}
...
fn validate_dim_limits() {
    ennx::backend::disk_observation::validate_dim_limits(4, 1).unwrap();
}
...
fn parse_json_usize_field() {
    ...
    assert_eq!(
        ennx::backend::disk_observation::parse_json_usize_field(text, "num_obs"),
        Some(42)
    );
}
