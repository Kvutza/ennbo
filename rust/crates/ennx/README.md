...
```bash
ENNX_WORK_DIR=/tmp/enn_work cargo test -p ennx disk_bpann
```
...
```rust
use ndarray::array;
use ennx::{EpistemicNearestNeighbors, ENNParams, index::IndexDriver};
...
The [ennx](https://pypi.org/project/ennx/) Python package provides PyO3 bindings:

```bash
pip install ennx[with-deps]
```
